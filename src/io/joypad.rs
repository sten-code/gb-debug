    ...
    pub fn read_byte(&self) -> u8 {
        ...
        match self.selected {
            ...
            Selected::Both => {
                ...
                0b1100_0000 | start_bit | select_bit | b_bit | a_bit | down_bit | up_bit | left_bit | right_bit
            }
...
mod test {
    ...
    fn keys_buttons() {
        ...
        for i in 0..4 {
            ...
            assert_eq!(joypad.read_byte(), 0xCF, "i: {}, selected: {:?}", i, joypad.selected);

            joypad.write_byte(0x10);
            assert_eq!(joypad.read_byte(), 0xDF & !(1 << i), "i: {}, selected: {:?}", i, joypad.selected);

            joypad.write_byte(0x20);
            assert_eq!(joypad.read_byte(), 0xEF, "i: {}, selected: {:?}", i, joypad.selected);

            joypad.write_byte(0x30);
            assert_eq!(joypad.read_byte(), 0xFF, "i: {}, selected: {:?}", i, joypad.selected);
    ...
    fn keys_direction() {
        ...
        for i in 0..4 {
            ...
            joypad.write_byte(0x10);
            assert_eq!(joypad.read_byte(), 0xDF, "i: {}, selected: {:?}", i, joypad.selected);

            joypad.write_byte(0x20);
            assert_eq!(joypad.read_byte(), 0xEF & !(1 << i), "i: {}, selected: {:?}", i, joypad.selected);
